 ...
 * Respective controllers for Lifeline nodes and Message edges are assigned. The
 * handling of the configuration is taken from the base class
 ...
 */
public class UMLGraphController extends ActorViewerGraphController implements
		ChangeListener, ValueListener {
	 ...
	 * 
	 * The width of the CombinedFragment depends on the most left and most right
	 * Lifeline on the screen.
	 * 
	 * @author Andreas Thuy
	 * 
	 */
	public class CreateCombFragAction extends AbstractAction {
		 ...
		 * 
		 * @param buttonText
		 *            - this String will appear on the button that is associated
		 *            with this action.
		 */
		...
		// TODO after 'intelligent' changes (08.09.06), lifelines are no longer
		// centered
		// witht he CombinedFragment...
		/**
		 * Called when someone 'activates' this action. A CombinedFragment will
		 * be created and added to the Ptolemy-model.
		 * 
		...
		public void actionPerformed(ActionEvent event) {
			if (getGraphModel().getRoot() instanceof NamedObj) {
					...
					if (!lifelines.isEmpty()) {
						for (int i = 0; i < lifelines.size(); i++) {
							...
							double lifelineX = lifeline.getLocation()
									.getLocation()[0];
							// TODO this lifelineFigure has to be removed...
							// problems with LifelineX
							LifelineFigure l = (LifelineFigure) UMLGraphController.this
									.getFigure(lifeline.getLocation());
				...
				getGraphModel().dispatchGraphEvent(
	...
	public class MessageAction extends FigureAction {
		...
		public void actionPerformed(ActionEvent e) {
			// this is an Action, but it will actually only react to
			// JToggleButtons
			...
			CompositeInteractor c = (CompositeInteractor) getLifelineController()
					.getNodeInteractor();
			...
			if (msgCreator == null || dragger == null) {
				System.out
						.println("Problems with Interactors of Lifeline nodes!");
			} else {
	...
	/**
	 * Interactor that is used when a message is created. No changes are applied
	 * to the Ptolemy-model yet. Only graphical operations are used here.
	 * 
	 * @author Andreas Thuy
	 */
	...
	public class MessageCreator extends AbstractInteractor {
		 ...
		 * (non-Javadoc)
		 * 
		 * Create the MessageConnector that will be shown on the screen. Only
		 * create the figure but no changes to the Ptolemy-model.
		 * 
		 * Do only start to draw the Message if the mouse was pressed on a
		 * Lifeline.
		 * 
		 * @see
		 * diva.canvas.interactor.AbstractInteractor#mousePressed(diva.canvas
		 * .event.LayerEvent)
		 */
			...
			if (source != null) {
				if (source instanceof LifelineFigure) {
					...
					if (((LifelineFigure) source).isOnLiveLine(p)) {
						...
						// :)
						// source:
						// ActorEditorGraphController.LinkCreator.mousePressed(...)
						...
						tailSite = new AutonomousSite(layer,
								layerEvent.getLayerX(), layerEvent.getLayerY());
						...
						tailSite = getMessageController().getConnectorTarget()
								.getTailSite(c, source, layerEvent.getLayerX(),
										layerEvent.getLayerY());
						if (tailSite == null) {
						...
						ConnectorManipulator cm = (ConnectorManipulator) c
								.getParent();
		...
		/**
		 * @param isSynchronous
		 *            - state if Messages of MessageSort.SYNCH_MESSAGE (true) or
		 *            MessageSort.ASYNCH_MESSAGE should be created
		 */
	 ...
	 * 
	 * @see
	 * ptolemy.vergil.basic.BasicGraphController#addToMenuAndToolbar(javax.swing
	 * .JMenu, javax.swing.JToolBar)
	 */
	...
	public void addToMenuAndToolbar(JMenu menu, JToolBar toolbar) {
		...
		try {
			...
			asyncURL = new URL("file:///" + System.getProperty("user.dir")
					+ "/ptolemy/vergil/uml/img/asyncMessage.gif");
			syncURL = new URL("file:///" + System.getProperty("user.dir")
					+ "/ptolemy/vergil/uml/img/syncMessage.gif");
			combURL = new URL("file:///" + System.getProperty("user.dir")
					+ "/ptolemy/vergil/uml/img/combFrag.gif");

			// asyncURL = new
			// URL("file:///D:/Code/ptII/ptolemy/vergil/uml/img/asyncMessage.gif");
			// syncURL = new
			// URL("file:///D:/Code/ptII/ptolemy/vergil/uml/img/syncMessage.gif");
			// combURL = new
			// URL("file:///D:/Code/ptII/ptolemy/vergil/uml/img/combFrag.gif");
			// asyncURL = new
			// URL("file:/home/maatta/ptII6.0.2/ptolemy/vergil/uml/img/asyncMessage.gif");
			// syncURL = new
			// URL("file:/home/maatta/ptII6.0.2/ptolemy/vergil/uml/img/syncMessage.gif");
			// combURL = new
			// URL("file:/home/maatta/ptII6.0.2/ptolemy/vergil/uml/img/combFrag.gif");
		...
		JButton combFragButtonLoop = new JButton(new CreateCombFragAction(
				"Loop"));
	...
	public NodeController getNodeController(Object object) {
		...
		if (object instanceof Location) {
			...
			} else if (((Location) object).getContainer() instanceof CombinedFragment) {
	...
	protected void updateCombFragPos() {
		// calculate width of the CombinedFragment so that it
		// includes all the lifelines from left to right
		List lifelines = ((NamedObj) UMLGraphController.this.getGraphModel()
				.getRoot()).attributeList(Lifeline.class);
		double leftX = 0;
		double rightX = 0;
		double x = 0;
		double width = 0;
		if (lifelines != null) {
			...
			for (int i = 0; i < lifelines.size(); i++) {
				...
				// TODO this lifelineFigure has to be removed... problems with
				// LifelineX
				LifelineFigure l = (LifelineFigure) UMLGraphController.this
						.getFigure(lifeline.getLocation());
				lifelineX = l.getLifelineX();
	 ...
	 * 
	 * @see
	 * ptolemy.kernel.util.ValueListener#valueChanged(ptolemy.kernel.util.Settable
	 * )
	 * 
	 ...
	 * ExternalIOPortController._GENERIC_INPUT, "New input port", KeyEvent.VK_I,
	 * new String[][] { { "/ptolemy/vergil/actor/img/single_in.gif",
	 * GUIUtilities.LARGE_ICON }, { "/ptolemy/vergil/actor/img/single_in_o.gif",
	 * GUIUtilities.ROLLOVER_ICON }, {
	 * "/ptolemy/vergil/actor/img/single_in_ov.gif",
	 * GUIUtilities.ROLLOVER_SELECTED_ICON }, {
	 * "/ptolemy/vergil/actor/img/single_in_on.gif", GUIUtilities.SELECTED_ICON
	 * } });
	 */
	 ...
	 * "/ptolemy/vergil/actor/img/single_out.gif", GUIUtilities.LARGE_ICON }, {
	 * "/ptolemy/vergil/actor/img/single_out_o.gif", GUIUtilities.ROLLOVER_ICON
	 * }, { "/ptolemy/vergil/actor/img/single_out_ov.gif",
	 * GUIUtilities.ROLLOVER_SELECTED_ICON }, {
	 * "/ptolemy/vergil/actor/img/single_out_on.gif", GUIUtilities.SELECTED_ICON
	 * } });
	 */
