...
public class MessageController extends BasicEdgeController {
	...
	protected class MessageDropper extends ConnectorAdapter {
		 ...
		 * 
		 * If the HEAD_END of a connector is dropped on a LifelineFigure, then a
		 * Message is completed and the according MoML description will be
		 * created
		 * 
				...
				if (figure != null && figure instanceof LifelineFigure) {
					...
					if (node != null && node instanceof Location) {
						if (((Location) node).getContainer() instanceof Lifeline) {
							...
							UMLSeqActor rootModel = (UMLSeqActor) getController()
							...
							if (connector.getTailSite() != null) {
								...
								if (tailSite.getFigure() != null) {
									if (tailSite.getFigure().getUserObject() != null) {
										senderLifeline = (Lifeline) ((Location) connector
							...
							Lifeline receiverLifeline = (Lifeline) ((Location) node)
									.getContainer();
							if (senderLifeline != null
								...
								try {
									...
									CompositeInteractor compInt = (CompositeInteractor) ((UMLGraphController) getController())
											.getLifelineController()
											.getNodeInteractor();
									Iterator interactors = compInt
											.interactors();
									while (interactors.hasNext()) {
										Interactor inter = (Interactor) interactors
												.next();
										if (inter.isEnabled()) {
											if (inter instanceof MessageCreator) {
									...
									Message msg = new Message(rootModel, name,
											msgSort, null, null);
									Iterator mOccSpecs = msg
											.attributeList(
													MessageOccurrenceSpecification.class)
											.iterator();
									while (mOccSpecs.hasNext()) {
										NamedObj msgOccSpec = (NamedObj) mOccSpecs
												.next();
										if (msgOccSpec.getName().equals(
												"sendEvent")) {
											double[] senderLoc = senderLifeline
													.getLocation()
													.getLocation();
											// obtain MoML description
											String msgOccSpecString = getMsgOccSpecMoML(
													senderLifeline
															.getRepresents(),
													...
													(new Double(sendingTime[1]))
															.toString());
											MoMLChangeRequest adjustMOccSpec = new MoMLChangeRequest(
													msgOccSpec, msgOccSpec,
													msgOccSpecString);
											msgOccSpec
													.requestChange(adjustMOccSpec);

										} else if (msgOccSpec.getName().equals(
												"receiveEvent")) {
											double[] receiverLoc = receiverLifeline
													.getLocation()
													.getLocation();
											// obtain MoML description
											String msgOccSpecString = getMsgOccSpecMoML(
											...
											MoMLChangeRequest adjustMsgOccSpec = new MoMLChangeRequest(
													msgOccSpec, msgOccSpec,
													msgOccSpecString);
											msgOccSpec
													.requestChange(adjustMsgOccSpec);
											...
											if (isSync) {
												...
												while (returnMOSs.hasNext()) {
													NamedObj returnMOS = (NamedObj) returnMOSs
															.next();
													if (returnMOS
															.getName()
															.equals("sendEvent")) {
														double[] senderLoc = receiverLifeline
																.getLocation()
																.getLocation();
														String returnMsgOccSpecString = getMsgOccSpecMoML(
																receiverLifeline
																		.getRepresents(),
																(new Double(
																		senderLoc[0]))
														...
														MoMLChangeRequest adjustReturnMOS = new MoMLChangeRequest(
																returnMOS,
																returnMOS,
																returnMsgOccSpecString);
														returnMOS
																.requestChange(adjustReturnMOS);

													} else if (returnMOS
															.getName()
															.equals("receiveEvent")) {
														double[] returnReceiverLoc = senderLifeline
																.getLocation()
																.getLocation();
														String returnMsgOccSpecString = getMsgOccSpecMoML(
																senderLifeline
							...
							System.out
									.println("Only Lifelines are supported...!");
						}
					} else {
					...
					getController().getSelectionModel().removeSelection(
							connector);
					CanvasLayer l = connector.getLayer();
					((FigureLayer) l).remove(connector);
					l.repaint();
		...
		public void connectorUnsnapped(ConnectorEvent e) {
			...
			if (con != null) {
				if (e.getEnd() == ConnectorEvent.HEAD_END) {
					...
					// setup of the connector is expected (tailSite and
					// headSite)
					con.setHeadSite(new AutonomousSite(getController()
							.getGraphPane().getForegroundLayer(), con
							.getHeadSite().getX(), con.getTailSite().getY()));
				}
			}
		}
	...
	/**
	 * This class is responsible for the visual representation of a message in a
	...
	public class MessageRenderer implements EdgeRenderer {
		 ...
		 * (non-Javadoc)
		 * 
		 * @see diva.graph.EdgeRenderer#render(java.lang.Object,
		 * diva.canvas.Site, diva.canvas.Site)
		 */
		public Connector render(Object edge, Site tailSite, Site headSite) {
			...
			if (edge instanceof Message) {
				...
				if (straightMessage != null) {
					...
					straightMessage.setStartPoint(msg.getSendEvent()
							.getLocTime()[1]);
				}
			} else {
				System.out
						.println("I can only draw messages, and nothing else!");
			}
		 ...
		 * Ptolemy model yet, but a Message shall be drawn, this method returns
		 * a dummy MessageConnector that has to be replaced later on with one
		 * that contains a UserObject.
		 * 
	 ...
	 * attached. If an appropriate target is found, a Site on the target can be
	 * returned which will be the headSite of the MessageConnector.
	 * 
	 * @author Andreas Thuy
	 */
	public class MessageTarget extends AbstractConnectorTarget {
		...
		// an argument is used instead, however, this method must be
		// implemented...
	 ...
	 * clicking 'Rename' menu on messages. Assign a ConnectorManipulator and
	 * make that messages are rendered as MessageConnectors.
	 * 
	 * @param controller
	...
	public MessageController(GraphController controller) {
		...
		/*
		 * { // now it's really enough!!!!!!!! // if my right mouse button is
		 * not recognized as a popup-trigger // then, I just don't test it!!!
		 * enough is enough public void mousePressed(LayerEvent e) {
		 * if(getMenuFactory() != null) { Figure source = e.getFigureSource();
		 * JPopupMenu menu = getMenuFactory().create(source);
		 * 
		 * if (menu == null) { return; }
		 * 
		 * menu.show(e.getComponent(), e.getX(), e.getY()); e.consume(); } } }
		 */
		;
				...
				if (getHandle().getSite() == getConnector().getHeadSite()) {
					Figure figure = container.pick(hitRect, new Filter() {
						public boolean accept(Object o) {
							...
							try {
								transform = figureContext.getTransform(
										containerContext).createInverse();
							} catch (Exception ex) {
								ex.printStackTrace();
								return false;
							}

							Rectangle2D bounds = ShapeUtilities
									.transformBounds(hitRect, transform);

							if (getConnectorTarget().getHeadSite(
									getConnector(), f, bounds.getCenterX(),
					...
					if (figure != null) {
						Site snap = getConnectorTarget().getHeadSite(figure,
								hitRect.getCenterX(),
	...
	public Figure drawEdge(Object edge) {
		...
		// TODO really?
		MessageOccurrenceSpecification tail = (MessageOccurrenceSpecification) model
				.getTail(edge);
		MessageOccurrenceSpecification head = (MessageOccurrenceSpecification) model
				.getHead(edge);
		...
		try {
			tailLifeline = (Lifeline) container.getAttribute(tailName,
					Lifeline.class);
			headLifeline = (Lifeline) container.getAttribute(headName,
					Lifeline.class);
		} catch (IllegalActionException iAE) {
			iAE.printStackTrace();
		}
		...
		if (tailLifeline != null && headLifeline != null) {
			...
			headSite = getConnectorTarget()
					.getHeadSite(headFigure,
							headLifeline.getLocation().getLocation()[0],
							receiveTime[1]);
