 ...
 * simplified here. For example, InteractionFragment that is the super class of
 * CombinedFragment is not implemented here.
 * 
 ...
 * InteractionOperands do only consist of one Message in this implementation and
 * therefore InteractionOperands are implicitly created for every single
 * Message.
 * 
...
public class CombinedFragment extends Parameter {
	...
	public CombinedFragment(NamedObj container, String name, double[] loc,
		...
		if (iOp.getOperator() == "loop" || iOp.getOperator() == "alt"
				|| iOp.getOperator() == "opt") {
			createCFPorts_();
		}
	...
	/**
	 * @return - the name of the InteractionOperator that is assigned to the
	 *         CombFrag
	 */
	public String getOp() {
	 ...
	 * 
	 * @return - the position of the Combined Fragment on the screen as a double
	 *         table (x and y coordinates can be extracted easily)
	 */
	...
	private void createCFPorts_() {
		try {
			...
			inputport_ = new CombinedFragmentPort(patternholder, getName()
					+ "_input", true, false, this);

			String inputPortMoMLRep = "<port name=\"" + inputport_.getName()
					+ "\" class=\"lsi.noc.application.CombinedFragmentPort\">"
					+ "  <property name=\"input\"/>" + "  <property name=\""
					+ this.getName()
					+ "\" class=\"ptolemy.vergil.uml.CombinedFragment\" />"
