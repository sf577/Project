...
/**
 * This is a top-level window which enables the view of a PtolemyII model as a
 * UML sequence diagram. The Look & Feel is based on the graphical
...
public class UMLFrame extends BasicGraphFrame {
	...
	/**
	 * This frame is controlled by a UMLGraphController and the underlying model
	 * corresponds to an ActorGraphModel.
	 * 
	 * @return a GraphPane that is controlled by a UMLGraphController and that
	 *         is based on an ActorGraphModel
	...
	protected GraphPane _createGraphPane(NamedObj abu) {
		...
		// ActorGraphModel model = new
		// ActorGraphModel((CompositeEntity)getModel());
	...
	public void delete() {
		...
		UMLGraphModel graphModel = (UMLGraphModel) controller.getGraphModel();
		...
		for (int i = 0; i < selection.length; i++) {
			...
			if (graphModel.isNode(userObject)) {
				...
				Iterator msgsToDelete = graphModel.getLifelineModel().outEdges(
						userObject);
				...
				msgsToDelete = graphModel.getLifelineModel()
						.inEdges(userObject);
				while (msgsToDelete.hasNext()) {
					Message msgToDelete = (Message) msgsToDelete.next();
					moml.append(graphModel.getDeleteEdgeMoML(msgToDelete));
