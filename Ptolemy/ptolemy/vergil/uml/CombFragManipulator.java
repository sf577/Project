	...
	public void mouseReleased(LayerEvent e) {
		...
		if (f instanceof BasicGrabHandle) {
			if (f.getParent() instanceof CombFragManipulator) {
				CombFragFigure combFigure = (CombFragFigure) ((CombFragManipulator) f
						.getParent()).getChild();
				...
				if (combYValue < e.getY()) {
					...
					if (locList.size() == 1) {
						...
						double newY = ((combYValue - combi.getHeigth() / 2) + e
								.getY()) / 2;
						double newHeight = (e.getY() - newY) * 2;
						double[] newLoc = {
								combi.getLocation().getLocation()[0], newY };
						...
						buf = "</group>";
						momelString.append(buf);
						combi.getContainer().requestChange(
								new MoMLChangeRequest(combi.getContainer(),
										combi.getContainer(), momelString
												.toString()));
					}
				} else {
					List locList = combi.attributeList(Location.class);
