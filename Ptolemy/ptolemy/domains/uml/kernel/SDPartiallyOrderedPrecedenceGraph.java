...
public class SDPartiallyOrderedPrecedenceGraph extends SDPrecedenceGraph {
	...
	Hashtable _mapping; // stores all MessageOccurrenceSpecification of each
						// Lifeline using a TreeSet
	Hashtable _state; // stores for each Lifeline which
						// MessageOccurrenceSpecification can occurr next
	...
	private void _assignMessageToLifeline(MessageOccurrenceSpecification m) {
		...
		v.add(m);
		if (m == v.first()) {
			_state.put(p, m); // if the MessageOccurrenceSpecification is the
								// first one on the Lifeline, it is the one
								// allowed to occur
		}
	...
	public void incrementState(Lifeline l) {
		...
		// set the next state to the next MessageOccurrenceSpecification in the
		// Lifeline
		try {
			next = (MessageOccurrenceSpecification) v.elementAt(ind + 1);
		} catch (ArrayIndexOutOfBoundsException e) {

			// if MessageOccurrenceSpecification is the last in a Lifeline, put
			// the first again
