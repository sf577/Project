 ...
 * array, which shows during run-time the average dynamic power dissipation of
 * each buffer regarding the received transmission rate defined inside a given
 * sample period. Copyright (c) 2007 - All rights reserved. <br>
 * 
...
public class PowerScopeActor extends NoCScopeActor {
	...
	public void fire() throws IllegalActionException {
		...
		/*
		 * try{
		 * dataOutput.writeBytes("==================== Power Estimation at Time = "
				...
				for (int i = 0; i < width; i++) {
					if (p.hasToken(i)) {
						...
						if (t instanceof RecordToken) {
							...
							double e = ((DoubleToken) record.get("e"))
									.doubleValue();
							double w = ((DoubleToken) record.get("w"))
									.doubleValue();
							double n = ((DoubleToken) record.get("n"))
									.doubleValue();
							double s = ((DoubleToken) record.get("s"))
									.doubleValue();
							double local = ((DoubleToken) record.get("l"))
									.doubleValue();

							// Get the router Type
							String routerType = getRouterType(port,
									((width - 1) - i), width, width);

							double AvRPC = 0;

							if (routerType == "RouterCC") {
								double avRR = (e + w + n + s + local) / 5;
								AvRPC = (getBufferAPC(e) + getBufferAPC(w)
										+ getBufferAPC(n) + getBufferAPC(s) + getBufferAPC(local))
										+ getAvSwitchCrtl(avRR);
								setRouterAvPC(AvRPC);

								if (PS_visible) {
									// Fill out the Average Power Consumption of
									// each Buffer in the PowerScope
									((PowerScope) _nocscope).getSwitch(port, i)
											.setEast(druncate(getBufferAPC(e)));
									((PowerScope) _nocscope).getSwitch(port, i)
											.setWest(druncate(getBufferAPC(w)));
									((PowerScope) _nocscope)
											.getSwitch(port, i)
											.setNorth(druncate(getBufferAPC(n)));
									((PowerScope) _nocscope)
											.getSwitch(port, i)
											.setSouth(druncate(getBufferAPC(s)));
									((PowerScope) _nocscope)
											.getSwitch(port, i)
											.setLocal(
													druncate(getBufferAPC(local)));
								}
							} else if (routerType == "RouterTC") {
								double avRR = (e + w + s + local) / 4;
								AvRPC = (getBufferAPC(e) + getBufferAPC(w)
										+ getBufferAPC(s) + getBufferAPC(local))
										+ getAvSwitchCrtl(avRR);
								setRouterAvPC(AvRPC);
								...
								AvRPC = (getBufferAPC(e) + getBufferAPC(n)
										+ getBufferAPC(s) + getBufferAPC(local))
										+ getAvSwitchCrtl(avRR);
								setRouterAvPC(AvRPC);
								if (PS_visible) {
								...
								AvRPC = (getBufferAPC(w) + getBufferAPC(s)
										+ getBufferAPC(n) + getBufferAPC(local))
										+ getAvSwitchCrtl(avRR);
								setRouterAvPC(AvRPC);
								if (PS_visible) {
									((PowerScope) _nocscope).getSwitch(port, i)
								...
								AvRPC = (getBufferAPC(e) + getBufferAPC(s) + getBufferAPC(local))
										+ getAvSwitchCrtl(avRR);
								setRouterAvPC(AvRPC);
								if (PS_visible) {
									((PowerScope) _nocscope).getSwitch(port, i)
								...
								AvRPC = (getBufferAPC(w) + getBufferAPC(s) + getBufferAPC(local))
										+ getAvSwitchCrtl(avRR);
								setRouterAvPC(AvRPC);
								if (PS_visible) {
									((PowerScope) _nocscope).getSwitch(port, i)
								...
								AvRPC = (getBufferAPC(e) + getBufferAPC(n) + getBufferAPC(local))
										+ getAvSwitchCrtl(avRR);
								setRouterAvPC(AvRPC);
								if (PS_visible) {
									((PowerScope) _nocscope).getSwitch(port, i)
								...
								AvRPC = (getBufferAPC(w) + getBufferAPC(n) + getBufferAPC(local))
										+ getAvSwitchCrtl(avRR);
								setRouterAvPC(AvRPC);
								if (PS_visible) {
									((PowerScope) _nocscope).getSwitch(port, i)
							...
							/*
							 * try{ dataOutput.writeBytes("Router"+ port + i+
							 * " Average Consumption "+ AvRPC + "\n"); } catch
							 * (IOException ex) {}
							 */
	...
	/*
	 * Buffer Average Power Consumption - For each reception rate, the
	 * associated power dissipation is calculated regarding the buffer_VD and
	 * buffer_BD. where: buffer_VD = buffer variable dissipation buffer_BD =
	 * buffer base dissipation
	 */
	...
	private double getAvRouterRate(double e, double w, double s, double n,
			double local, int ports) {
	...
	/**
	 * @Function: Returns the ROUTER TYPE e.g. RouterTL - router located on the
	 *            TOP LEFT side of the NoC)
	 */
	...
	public void wrapup() throws IllegalActionException {
		try {
			dataOutput
					.writeBytes("==================== Power Estimation at Time = "
							+ getDirector().getModelTime()
							+ " ====================\n");
			...
			for (int i = 0; i < avRoterPC.size(); i++) {
				...
				dataOutput.writeBytes("Router" + i + " Average Consumption "
						+ av_routerPC + "\n");
			}
			dataOutput.close();
		} catch (IOException e) {
