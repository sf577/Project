...
public class VCProducerCB extends VCProducer {
		...
		if (ack_in.hasToken(vc)) {
			...
			if (_debugging)
				_debug(getDirector().getModelTime()
						+ " credit update received: " + credits);
	...
	public void fire() throws IllegalActionException {
		...
		if (!busy_ && buffer_.size() > 0) {
			...
			if (_debugging)
				_debug("setting packet to be sent - size: " + size_);

		}

		if (busy_ && credits != 0
				&& (-1 == currentTime_.compareTo(getDirector().getModelTime()))) {
			...
			if (_debugging)
				_debug(currentTime_ + " packet sent - credits: " + credits);
	...
	public void sendPacket(Token token, int x, int y, int id, int size,
		...
		if (!busy_) {
			...
			if (_debugging)
				_debug("a token should be sent out");
