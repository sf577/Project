 ...
 * 
 * @author Sanna Maatta
 * @version 2.2, pipelined application execution possible.
 * 
...
public class TotalOrderPrecedenceGraph extends
		ptolemy.domains.uml.kernel.SDPrecedenceGraph {

	public TotalOrderPrecedenceGraph(List lifelines, List messages,
			List fragments) {
	 ...
	 * executed or not. The precedence is satisfied if all previous nodes in the
	 * graph that should be fired before this message are fired (false opt and
	 * alt conditions may have left some nodes not fired, which is ok).
	 */
	public boolean precedenceSatisfied(Message m) {

		// If all nodes in the graph are fired, on execution round of the
		// sequence diagram has finished and all nodes are set as not
		// fired waiting for the next execution round to begin.
		if (allNodesFired_()) {
		...
		if (node.getFired() || !allPreviousNodesFired_(node)) {
			...
			if (!parallel_) {
				...
				// If the node is inside the same parallel fragment, it can be
				// fired even if the previous one is not yet back from the
				// network
	 ...
	 * can be executed or not. The precedence is satisfied if all previous nodes
	 * in the graph that should be fired before this message are fired (false
	 * opt and alt conditions may have left some nodes not fired, which is ok).
	 */
	...
	/**
	 * This method adds edges for the nodes. The table below indicates in which
	 * situations an edge between the nodes p and c is created.
	 * 
	...
	protected void addEdges_() {
		...
		while (it.hasNext()) {
			...
			currentNode = (PrecedenceGraphNode) it.next();
			...
			// Lines 2, 3, 5, 6, 7: First fragment is null, so the previous node
			// is either a normal node or null
			if (firstFragment == null) {
				...
				if (op != null && op.equals("par")) {
					...
					// Lines 2, 5, and 6: nothing to be done in case of lines 2
					// or 5.
				} else {

					// Line 6: Edge created, previous and current nodes are
					// normal, not null, not inside a parallel combined fragment
				...
				if (op != null && op.equals("par")) {
					...
					if (firstFragment == fragment) {
						...
						// Adding the node in the TreeSet holding the parallel
						// nodes, no edges created.
						parallelFirst.add(currentNode.getMessage());
					...
					// Line 10: Next to parallel is not inside a par combined
					// fragment
				} else {
				...
				if (op != null && op.equals("par")) {
					...
					if (secondFragment == fragment) {
						...
						// Line 16: The current node is inside a 3rd adjacent
						...
						// to the 3rd adjacent parallel fragment can be placed
						// into the second treeset.
						while (iter.hasNext()) {
					...
					// Creating an edge from all the nodes in the second tree
					...
					while (iter1.hasNext()) {
						...
						// Creating edges from the nodes in the first treeset to
						// all of the nodes in the second treeset
						while (iter2.hasNext()) {
				...
				System.out
						.println("BIG PROBLEM! TotalOrderPrecedenceGraph should never come here");
			}
		...
		// Line 9: Current node is null, but previous has been inside a parallel
		// fragment
		if (firstFragment != null && secondFragment == null) {

			// Checking that there is a normal node before the parallel fragment
			if (previousToParallel != null) {
			...
			// Edge from all nodes from the first TreeSet to all nodes in the
			// second TreeSet
			while (iter1.hasNext()) {
