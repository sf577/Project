...
public class TotalOrder extends MappableSDDirector {
	/**
	 ...
	 *                specified name.
	 */
	public TotalOrder(CompositeEntity container, String name)
	 ...
	 */
	public TotalOrder(Workspace workspace) {
		super(workspace);
	...
	/**
	 * Creates a new total order precedence graph.
	 */
	public void createPrecedenceGraph() {
		...
		if (container_ instanceof UMLSeqActor) {
			...
			_graph = new TotalOrderPrecedenceGraph(lifelines, messages,
					fragments);
		}
	}
	...
	private boolean callTransferOutputs_(Message m)
		...
		for (int i = 0; i < outputPorts.size(); i++) {
			...
			if (io.hasTokenInside(0)) {
				...
				if (io instanceof BehavioralPatternPort) {
					transferOutputs(io);
				}
			}
		}
