...
public class ProcessControlBlock {
	...
	// TODO SHOULD BE FIXED TO USE Ptolemy's Time object
	// rather than rounding down times
	public Boolean hasFinished(double elapsedTime) {
		...
		if (_debug)
			System.out.println("Process id " + this.id
					+ "    usedComputation = " + usedComputation);
		if (_debug)
			System.out.println("Process id " + this.id
	...
	/*
	 * public Boolean hasFinished(double elapsedTime) { // Update the time the
	 * process has executed usedComputation = usedComputation + elapsedTime;
	 * double remainingTime = totalComputation - usedComputation;
	 * 
	 * if (_debug) System.out.println("Process id " + this.id +
	 * " elapsedTime = " + elapsedTime); if (_debug)
	 * System.out.println("Process id " + this.id + " usedComputation = " +
	 * usedComputation); if (_debug) System.out.println("Process id " + this.id
	 * + " totalComputation = " + totalComputation);
	 * 
	 * // If remaining computation is lower or equal to zero, process has
	 * finished if (remainingTime <= 0) { return true; } else { // However,
	 * oepending on the current precision of the simulation (see the director),
	 * // the remaining time should be disconsidered, as Ptolemy will not be
	 * able to // fire an event with such small precision (it will round it to
	 * 0) if (remainingTime < 0.00000001) { return true; } else { return false;
	 * } } }
	 */
