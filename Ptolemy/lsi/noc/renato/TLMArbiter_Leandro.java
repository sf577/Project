...
public class TLMArbiter_Leandro extends TypedAtomicActor {
	...
	public TLMArbiter_Leandro(CompositeEntity container, String name)
			...
			if (state[i] == REQUESTGRANTED) {
				if (inputreq[i].hasToken(0)) {
					if (!hasAck) {
						...
						// updates the state of the switch, discard input and
						// notify buffer
						state[i] = SIZE;
						if (_debugging)
							_debug(i + "->" + muxin[i] + " going to size state");
						inputreq[i].get(0);
