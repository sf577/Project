 ...
 * 
 *          Reads files generated by earlier versions of mappers and feeds the
 *          data to an instance of BasicCommunicationLatencyAnalysis, then
		...
		if (children != null) {
			for (int i = 0; i < children.length; i++) {
				...
				for (int j = 0; j < tokenizedContents.length / 4; j++) {
					...
					Double lat = Double
							.parseDouble(tokenizedContents[4 * j + 3]);
					analiser.notifyReceipt(children[i].getName()/*
																 * .split(".")[0]
																 */, lat);
