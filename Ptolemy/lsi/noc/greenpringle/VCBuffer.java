...
public class VCBuffer extends TLMBuffer {
	...
	// Sets the number of virtual channels based on the width of the input
	// multiport.
	...
	// FIFO of the superclass is used as the FIFO of the first channel, so it
	// behaves
	...
	public void initialize() throws IllegalActionException {
		...
		if (o != i)
			throw new IllegalActionException(
					"incorrect virtual channel input/output ratio");
		if (a != i)
			throw new IllegalActionException(
					"incorrect virtual channel input/ack ratio");
		if (r != o)
			throw new IllegalActionException(
					"incorrect virtual channel output/read ratio");
	...
	public Object clone(Workspace workspace) throws CloneNotSupportedException {
		...
		try {
			...
			newObject.ack.setMultiport(true);
	...
	/**
	 * If there is an input on the <i>read</i> multiport, remove the oldest
	 * token from the respective queue.
	 * 
	...
	/**
	 * If there are new tokens on the <i>input</i> multiport, then put them in
	 * the respective FIFO.
	 * 
	 * For each queue, if it is not empty, send a copy of the oldest token on
	 * the queue to the <i>output</i> multiport.
	 * 
	 * 
	 * @exception IllegalActionException
	 */
	public void fire() throws IllegalActionException {
		...
		// This should happen at most once every period (determined by a
		// parameter
		// of the superclass), so a timestamp comparison is made at every fire.
		// comp=0 means that current time is exactly the time to send the next
		// copy
		// comp=1 means that current time after the time to send the next copy
		// comp=-1 means that current time is before the time to send the next
		// copy
		...
		if (comp == 0 | comp == 1) {
			...
			for (int i = 0; i < _queues.length; i++) {
				if (_queues[i].size() != 0) {
					...
					// I guess he is gathering together the flit and the buffer
					// occupation
					...
					RecordToken recordToken3 = RecordToken.merge(recordToken1,
							recordToken2);
