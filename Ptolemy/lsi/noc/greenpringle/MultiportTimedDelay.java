...
/**
 * This actor extends TimedDelay by redefining its ports to be multiports and by
 * changing its firing scheme to handle tokens arriving to the multiple channels
 * of the input port. All tokens are delayed by the same amount of time,
 * regardless of the channel they arrive. They will be sent to the output via
 * the same channel they arrived from.
 * 
 * 
 * 
	...
	public void fire() throws IllegalActionException {
		...
		for (int i = 0; i < input.getWidth(); i++) {
			...
			if (input.hasToken(i)) {
				...
				if (_debugging)
					_debug("vector size: " + vec.size());
				if (_debugging)
					_debug("read token on channel " + i);
				break;
		...
		if (_delayedOutputTokens.size() > 0) {
			...
			if (eventTime.equals(currentTime)) {
				...
				if (_debugging)
					_debug("wrote token on channel " + i);
				vec.remove(index + 1);
				vec.remove(index);
