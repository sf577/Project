...
public class BufferSwitch {
	...
	private int bigValue = 0, bigPort = 0;
	private int max = 8;
	...
	public void paint(Graphics2D g) {
		...
		g.draw(e);
		if (number)
			g.drawString(new Integer(ae).toString(), 74, 54);
		...
		g.draw(w);
		if (number)
			g.drawString(new Integer(aw).toString(), 6, 54);
		...
		g.draw(n);
		if (number)
			g.drawString(new Integer(an).toString(), 36, 20);
		...
		g.draw(s);
		if (number)
			g.drawString(new Integer(as).toString(), 36, 92);
		...
		g.draw(local);
		if (number)
			g.drawString(new Integer(alocal).toString(), 36, 54);
	}
	...
	public void setBigValue(int value) {
		bigValue = value;
	}

	public void setBigPort(int port) {
		bigPort = port;
	}
