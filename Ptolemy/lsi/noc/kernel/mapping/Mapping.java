...
public abstract class Mapping {
	...
	/*
	 * Abstract method to perform the mapping. Should be used by the subclasses
	 * to implement mapping algorithms. Returns false if mapping fails, true
	 * otherwise.
	 */
	public abstract boolean performMapping();

	/*
	 * An implementation of the method that maps tasks to processors according
	 * to a predefined mapping - Map in arguments.
	 */
	...
	/*
	 * Removes all previous mappings of the provided tasks and cores, then apply
	 * a predefined mapping (an instance of Map in arguments).
	 */
	...
	public Map<Task, ProcessingCore> getMapping() {
		return mapping;
	}

	public Map<Task, ProcessingCore> getMappingClone() {
		return ((Map<Task, ProcessingCore>) ((HashMap<Task, ProcessingCore>) mapping)
				.clone());
	}
	...
	/*
	 * Should be used by the subclasses to remove a task from a core. It sets
	 * the core entry of the task to null, removes the task from the list of
	 * tasks of the core and removes the task from the mapping table.
	 */
	protected void removeTaskFromCore(Task t, ProcessingCore pc) {

		if (((GroupableTask) t).getGroupStatus() == 1
				|| ((GroupableTask) t).getGroupStatus() == 0) {
	...
	/*
	 * Should be used by the subclasses to change a task from one core to
	 * another.
	 */
	/*
	 * Should be used by the subclasses to change a task from one core to
