...
public class PriorityApplicationModelRandom extends PriorityApplicationModel {
	...
	public PriorityApplicationModelRandom(int tasks, double minTaskTime,
			double maxTaskTime, double minTaskPeriod, double maxTaskPeriod,
			int tasksSeed, int flows, int minFlowSize, int maxFlowSize,
			int flowsSeed) {
		super();

		// TODO: We currently require that the number of tasks is greater than
		// the number of flows
		// More specifically, the source of the flows start from the first task
		// and so on (t1, t2, t3, ... tn)
	...
	protected void createRandomTasks(int n) {
		...
		// Random rndTaskPeriod = new
		// Random(Math.round(Math.ceil(maxTaskPeriod)));
		...
		for (int i = 0; i < n; i++) {
			...
			while (wcet < minTaskTime || wcet > maxTaskTime) {
				...
				// wcet =
				// Double.valueOf(twoDForm.format(rndTaskTime.nextDouble()));
			}

			// Create task with computation time and priority == -1 (priority
			// will be set later, on the assignment algorithm)
			PriorityTask task = new PriorityTask(-1, wcet);
			task.setName("Task" + i);

			// period between minTaskPeriod-maxTaskPeriod seconds, more or equal
			// than comptime
			do {
				...
				while (period > maxTaskPeriod || period < minTaskPeriod) {
					...
					// period =
					// Double.valueOf(twoDForm.format(rndTaskPeriod.nextDouble()));
				}
	...
	/*
	 * Create random flows of communication. NOTE: Currently, there is a
	 * restriction that the sending task communicates with only one task. NOTE:
	 * Moreover, there is another restriction that the period of the flow is the
	 * same of the sending task. NOTE: We model the worst-case response time of
	 * the sender task as the release jitter in the priority flow
	 */

	protected void createRandomFlows(int n) {
		Random rnd = new Random(randomFlowsSeed);

		// NOTE: This is used to implement the restriction that a task only
		// communicates with
