...
public class PriorityTask extends lsi.noc.kernel.GroupableTask {
	...
	public ArrayList<PriorityTask> getInterferenceSet(
			ArrayList<PriorityTask> allTasks) {
	...
	public double getResponseTime(ArrayList<PriorityTask> interferenceSet) {
		...
		do {
			...
			for (int i = 0; i < interferenceSet.size(); i++) {
				...
				BigDecimal mult = new BigDecimal((int) Math.ceil((wicopy / task
						.getPeriod())));
				interf = interf.add(mult.multiply(new BigDecimal(
						1000000000 * (task.getCompTime()))));
			}
			wi = new BigDecimal(1000000000 * getCompTime()).add(interf)
	...
	public boolean isGroupSchedulable(ArrayList<PriorityTask> taskSet) {
		...
		for (int m = 0; m < group.size(); m++) {
			double rt = getFromGroup(m).getResponseTime(
					getFromGroup(m).getInterferenceSet(taskSet));
			if (rt > getFromGroup(m).getDeadline()) {
				schedulable = false;
			}
		}

		return schedulable && isSchedulable(taskSet);// group array does not
														// include group head
