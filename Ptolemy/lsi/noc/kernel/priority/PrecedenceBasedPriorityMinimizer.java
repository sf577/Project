...
public class PrecedenceBasedPriorityMinimizer extends PriorityMinimizer {
	...
	public void createPriorityPrecedenceGraph() {
		...
		for (int i = 0; i < flows.size(); i++) {
			...
			// adds all directed edges to all traffics which should have higher
			// priority than flowi
	...
	public void visualizePriorityPrecedenceGraph() {
		...
		Layout<PriorityTrafficFlow, String> layout = new CircleLayout(
				priorityPrecedenceGraph);
		...
		Transformer<PriorityTrafficFlow, String> labeler = new Transformer<PriorityTrafficFlow, String>() {
			public String transform(PriorityTrafficFlow v) {
				return v.getSourceTask().getName() + "-"
						+ v.getDestinationTask().getName() + ": "
						+ priorities.get(v).toString();
	...
	// emulates a transitive reduction of the subgraph that contains the given
	// node,
	...
	public void applyTransitiveReduction(PriorityTrafficFlow current) {
		...
		if (suc == 0) {
			...
			Object[] preds = priorityPrecedenceGraph.getPredecessors(current)
					.toArray();
		...
		// if it has successors, check if all of them have priorities assigned
		// to them
		else {

			// if at least one hasn't, then recurse with that one
			// if all have priorities, assign the lowest priority +1 to the
			// current node, then recurse with a predecessor
			Object[] sucs = priorityPrecedenceGraph.getSuccessors(current)
					.toArray();
			int low = -1;
			boolean all = false;
			for (int i = 0; i < sucs.length; i++) {
				...
				if (pr == -1) {
					next = (PriorityTrafficFlow) sucs[i];
					all = false;
					break;
				} else {
					all = true;
			...
			if (all) {
				...
				Object[] preds = priorityPrecedenceGraph.getPredecessors(
						current).toArray();
				for (int i = 0; i < preds.length; i++) {
					int pr = priorities.get(preds[i]).intValue();
					if (pr == -1) {
