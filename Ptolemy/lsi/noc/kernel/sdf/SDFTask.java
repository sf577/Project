...
public class SDFTask extends SimpleTask {
	...
	public void setChannels(SDFCommunication[] inputs,
			SDFCommunication[] outputs, int[] consumptionRates,
			int[] productionRates) {
	...
	// Fires this task, consuming tokens from input channels and producing
	// tokens to output channels.
	// Throws an exception and returns false if there are not enough tokens on a
	// particular input.
	...
	public boolean fire() throws Exception {
		...
		for (int i = 0; i < inputs.length; i++) {
			...
			if (!b) {
				throw new Exception("Task " + this.getName()
