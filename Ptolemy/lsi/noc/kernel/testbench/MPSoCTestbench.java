...
public class MPSoCTestbench extends Thread {
	...
	double[][] rutilization; // utilization of each processor under random
								// mapping
	double[][] ffutilization; // utilization of each processor under first fit
								// mapping
	double[][] sffutilization; // utilization of each processor under first fit
								// spiral mapping
	double[][] offutilization; // utilization of each processor under optimized
								// first fit mapping
	double[][] soffutilization; // utilization of each processor under optimized
								// first fit spiral mapping
	double[][] nnutilization; // utilization of each processor under nearest
								// neighbor mapping
	double[][] onnutilization; // utilization of each processor under optimized
								// nearest neighbor mapping
	double[][] dutilization; // utilization of each processor under
								// designer-defined mapping
	...
	public void run() {
		...
		/*
		 * int count = 0; while(!rready){ System.out.println("not yet "+count);
		 * count++; rready = mapRandomOsmar(); }
		 */
		rready = mapRandom();
		...
		if (rready) {
			...
			// PriorityMinimizer pm = new
			// PrecedenceBasedPriorityMinimizer(flows);
		} else {
			System.out.println("Unschedulable under Random!");
		}
		...
		if (ffready) {
			...
			System.out.println("Unschedulable under First Fit!");
		}
		...
		if (sffready) {
			...
			System.out.println("Unschedulable under First Fit Spiral!");
		}
		...
		if (offready) {
			...
			System.out.println("Unschedulable under Optimized First Fit!");
		}
		...
		if (soffready) {
			...
			System.out
					.println("Unschedulable under Optimized First Fit Spiral!");
		}
		...
		if (nnready) {
			...
			System.out.println("Unschedulable under Nearest Neighbor!");
		}
		...
		if (onnready) {
			...
			System.out
					.println("Unschedulable under Optimized Nearest Neighbor!");
		}
		...
		if (dready) {
			...
			System.out.println("Unschedulable under Designer Defined!");
		}
	...
	public void report() {
		...
		for (int i = 0; i < tasks.size(); i++) {
			...
			double wcet = tasks.get(i).getResponseTime(
					tasks.get(i).getInterferenceSet(tasks));
			System.out.println(i + "," + name + "," + wcet + " "
					+ tasks.get(i).getCompTime());
		...
		for (int i = 0; i < flows.size(); i++) {
			...
			double wclatency = flows.get(i)
					.getDirectInterferenceWorstCaseLatency(trav, rout, flows);
			double iwclatency = flows.get(i).getWorstCaseLatency(trav, rout,
					flows);
			...
			System.out.println(i + "," + source + "," + dest + "," + dir + ","
					+ indir + "," + latency + "," + wclatency + ","
					+ iwclatency);

			// System.out.println("wcet "+
			// flows.get(i).getSourceTask().getWorstCaseExecutionTime(flows.get(i).getSourceTask().getInterferenceSet(tasks)));
	...
	public boolean mapRandom() {
		...
		for (int k = 0; k < tasks.size(); k++) {
			...
			for (int tr = 0; tr < 5; tr++) {
				...
				if (t.getGroupStatus() == 0
						&& rutilization[px][py] + t.getUtilization() <= 1) {
					rutilization[px][py] += t.getUtilization();
					t.setCore(pc);
					break;
				} else if (t.getGroupStatus() == 1
						&& rutilization[px][py] + t.getGroupUtilization() <= 1) {
					t.setCore(pc);
					rutilization[px][py] += t.getUtilization();
			...
			for (int px = 0; px < x; px++) {
				for (int py = 0; py < y; py++) {
					...
					for (int tr = 0; tr < 5; tr++) {
						...
						// NOTE: The utilization below 100% is used to see if
						// the thread is schedulable.
						// This is not correct, so later we do the Response-time
						// analysis to check if the scenario
						...
						if (t.getGroupStatus() == 0
							...
							break;
								...
								&& rutilization[px][py]
										+ t.getGroupUtilization() <= 1) {
							t.setCore(pc);
							rutilization[px][py] += t.getUtilization();
							for (int m = 0; m < t.group.size(); m++) {
								t.group.get(m).setCore(pc);
								rutilization[px][py] += t.group.get(m)
										.getUtilization();
							}
						} else if (tr == 4) {
							return false;
	...
	public boolean mapFirstFitSpiral() {
		...
		for (int k = 0; k < tasks.size(); k++) {
			...
			if (!doFirstFitSpiral(t, sffutilization)) {
				return false;
			}
	...
	private boolean doFirstFit(PriorityTask p, double[][] utilization) {
		...
		for (int i = 0; i < x; i++) {
			for (int j = 0; j < y; j++) {
				if (p.getGroupStatus() == 0
						&& utilization[i][j] + p.getUtilization() <= 1) {
					// assign task to the first processor that can accept it
					p.setCore(network.getCore(i, j));
					if (p.isSchedulable(tasks)) {
						System.out.println("Mapped task " + p.getName()
								+ " to core " + i + "," + j + " with util "
								+ utilization[i][j]);
						System.out
								.println("Period "
										+ p.getPeriod()
						...
						&& utilization[i][j] + p.getGroupUtilization() <= 1) {
					...
					if (p.isGroupSchedulable(tasks)) {
						...
						System.out.println("Mapped group head " + p.getName()
								+ " to core " + i + "," + j + " with util "
								+ utilization[i][j]);
						System.out
								.println("Period "
	...
	private boolean doFirstFitSpiral(PriorityTask p, double[][] utilization) {
		...
		ArrayList<ProcessingCore> cores = getNeighbors(new Point(
		...
		for (int i = 0; i < cores.size(); i++) {
			...
			if (p.getGroupStatus() == 0 && procutil + p.getUtilization() <= 1) {
				...
				return true;
					...
					&& procutil + p.getGroupUtilization() <= 1) {
				ProcessingCore pc = cores.get(i);
				p.setCore(pc);
				utilization[pos.x][pos.y] += p.getUtilization();
				for (int m = 0; m < p.group.size(); m++) {
					p.group.get(m).setCore(pc);
					utilization[pos.x][pos.y] += p.group.get(m)
							.getUtilization();
				}
				return true;
			}
	...
	public boolean mapOptimizedFirstFit() {
		...
		for (int i = 0; i < x; i++) {
			for (int j = 0; j < y; j++) {
				offutilization[i][j] = 0;
			}
		}

		// get a reordered task list so that high utilization ones are mapped
		// first
		...
		for (int k = 0; k < orderedTasks.size(); k++) {
			...
			if (!doFirstFit(t, offutilization)) {
				return false;
			}
	...
	public boolean mapOptimizedFirstFitSpiral() {
		...
		for (int k = 0; k < orderedTasks.size(); k++) {
			...
			PriorityTask t = orderedTasks.get(k);
			if (!doFirstFitSpiral(t, soffutilization)) {
				return false;
			}
	...
	public boolean mapNearestNeighbor() {
		...
		while (toMap.iterator().hasNext()) {
			...
			if (p.getGroupStatus() == 2) {
				...
				ArrayList<PriorityTask> comms = this
						.sortTasksbyPriority(communicatingTasks(p));

				ArrayList<ProcessingCore> neighbors;
				if (comms.size() > 0) {
					neighbors = getNeighbors(network.getMeshLocation(p
							.getCore()));
					...
					for (int i = 0; i < comms.size(); i++) {
						...
						if (pt.getGroupStatus() == 2) {
							toMap.remove(pt);
							mapped = true;
						}

						// map neighboring tasks to neighbors with lower hop
						// counts first
						for (int j = 0; j < neighbors.size(); j++) {

							Point pos = network.getMeshLocation(neighbors
									.get(j));
							double procutil = nnutilization[pos.x][pos.y];

							if (pt.getGroupStatus() == 0
									&& procutil + pt.getUtilization() <= 1) {
								// assign task to the first processor that can
								// accept it
								pt.setCore(neighbors.get(j));
								if (pt.isSchedulable(tasks)) {
									System.out.println("Mapped task "
											+ pt.getName() + " to core " + i
											+ "," + j + " with util "
											+ nnutilization[pos.x][pos.y]);
									System.out
											.println("Period "
													+ pt.getPeriod()
													+ " resp time "
													+ pt.getResponseTime(pt
															.getInterferenceSet(tasks)));
									nnutilization[pos.x][pos.y] += pt
											.getUtilization();
									toMap.remove(pt);
									mapped = true;
									break;
									...
									&& procutil + pt.getGroupUtilization() <= 1) {
								// assign task to the first processor that can
								// accept it
								pt.setCore(neighbors.get(j));
						...
						for (int j = 0; j < neighbors.size(); j++) {
							...
							if (pt.getGroupStatus() == 0
								...
								break;
									...
									&& procutil + pt.getGroupUtilization() <= 1) {
								// assign task group to the first processor that
								// can accept it
								onnutilization[pos.x][pos.y] += pt
										.getUtilization();
								pt.setCore(neighbors.get(j));
								...
								for (int m = 0; m < pt.group.size(); m++) {
									pt.group.get(m).setCore(neighbors.get(j));
									onnutilization[pos.x][pos.y] += pt.group
	...
	protected ArrayList<PriorityTask> sortTasksbyPriority(
			ArrayList<PriorityTask> ta) {
		...
		for (int k = 0; k < ta.size(); k++) {
			...
			if (orderedTasks.size() == 0) {
				orderedTasks.add(tasktoinsert);
			} else {
				boolean added = false;
				for (int i = 0; i < orderedTasks.size(); i++) {
					if (orderedTasks.get(i).getPriority() > tasktoinsert
							.getPriority()) {

						orderedTasks.add(i, tasktoinsert); // add right before
															// the 1st task with
															// lower priority
						added = true;
						break;
	...
	protected ArrayList<PriorityTask> sortTasksbyUtilization(
			ArrayList<PriorityTask> ta) {
		...
		for (int k = 0; k < ta.size(); k++) {
			...
			if (orderedTasks.size() == 0) {
				...
				boolean added = false;
				for (int i = 0; i < orderedTasks.size(); i++) {
					if (orderedTasks.get(i).getUtilization() < tasktoinsert
							.getUtilization()) {
	...
	// returns a new list of flows sorted by communication volume (highest
	// first)
	protected ArrayList<PriorityTrafficFlow> sortFlowsbyVolume(
			ArrayList<PriorityTrafficFlow> ta) {

		// reorder flows so that higher ratio of data over period are stored
		// first
		ArrayList<PriorityTrafficFlow> orderedFlows = new ArrayList<PriorityTrafficFlow>();
		for (int k = 0; k < ta.size(); k++) {

			PriorityTrafficFlow flowtoinsert = ta.get(k);
			double flowtoinsertratio = flowtoinsert.getPayload()
					/ flowtoinsert.getPeriod();
			if (orderedFlows.size() == 0) {
				orderedFlows.add(flowtoinsert);
			} else {
				boolean added = false;
				for (int i = 0; i < orderedFlows.size(); i++) {
					double ratio = orderedFlows.get(i).getPayload()
							/ orderedFlows.get(i).getPeriod();

					if (ratio < flowtoinsertratio) {

						orderedFlows.add(i, flowtoinsert); // add right before
															// the 1st flow with
															// lower ratio
						added = true;
						break;
	...
	public String getMappingString() {
		...
		for (int i = 0; i < x; i++) {
			for (int j = 0; j < y; j++) {
				cores[i][j] = new ArrayList<String>();
			}
		}
		...
		for (int i = 0; i < tasks.size(); i++) {
			...
			for (int c = 0; c < cores[p.x][p.y].size(); c++) {
				...
				if (s.equals(tasks.get(i).getName())) {
					contains = true;
				}
			}

			if (!contains) {
				cores[p.x][p.y].add(tasks.get(i).getName());
			}
