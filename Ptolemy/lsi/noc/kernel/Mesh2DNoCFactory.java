...
public class Mesh2DNoCFactory extends NoCFactory {
	...
	/**
	 * Generates a network-on-chip with at least <i>procs</i> processors and a
	 * square 2D mesh topology. The generated NoC will be the smallest square 2D
	 * mesh that can hold the number of processing cores passed as argument to
	 * the method.
	 * 
	 * @param procs
	...
	/**
	 * Generates a network-on-chip with a 2D mesh topology of width <i>x</i> and
	 * height <i>y</i>. The number of processing cores attached to the NoC will
	 * be <i>x*y</i>.
	 * 
	 * @param x
	 *            width of the 2D mesh
	 * @param y
	 *            height of the 2D mesh
	 * 
	 */
	...
	public Interconnect createInterconnect(int x, int y) {
		...
		for (int i = 0; i < x; i++) {
			...
			for (int j = 0; j < y; j++) {
				...
				// add output link to the west direction (except column where
				// i=0)
				if (i != 0) {
				...
				// add output link to the east direction (except column where
				// i=x-1)
				if (i != x - 1) {
				...
				// add output link to the north direction (except column where
				// j=y-1)
				if (j != y - 1) {
