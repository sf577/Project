 ...
 * <p>
 * The input can be of any token type, and the output is constrained to be of a
...
public class TLMBuffer extends Transformer {
	...
	/**
	 * Reacts to a change in an attribute. If the attribute is <i>capacity</i>,
	...
	/**
	 * If there is an input on the <i>read</i> port, remove the oldest token
	 * from the queue.
	 * 
	 ...
	 * FIFO.
	 * 
	 * If the queue is not empty, then send a copy of the oldest token on the
	 * queue to the <i>output</i> port.
	 * 
	 * <NOT ANYMORE,SINCE MOELLER TOUCHED IT> Send the resulting FIFO size to
	 * the <i>size</i> output port. </NOT ANYMORE,SINCE MOELLER TOUCHED IT>
	 * 
	 * @exception IllegalActionException
	...
	public void fire() throws IllegalActionException {
		...
		if (_queue.size() != 0 && (comp == 0 | comp == 1)) {
			...
			label[0] = "buffer_size";
			value[0] = new IntToken(_queue.size());
			RecordToken recordToken1 = new RecordToken(label, value);
			RecordToken recordToken2 = (RecordToken) _queue.get(0);
			RecordToken recordToken3 = RecordToken.merge(recordToken1,
					recordToken2);
			output.send(0, (Token) recordToken3);
			if (_debugging)
